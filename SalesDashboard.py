...
[data-testid="stAppViewContainer"] {
    background: linear-gradient(160deg, #fffef5 0%, #fdf8e8 50%, #fffef5 100%) !important;
}
...
/* Sidebar selectbox fix → white & gold */
...
.prod-bar { height: 6px; border-radius: 4px; background: linear-gradient(90deg, #b8860b, #f0c030); }
...
# ─── PLOTLY TEMPLATE ──────────────────────────────────────────────────────────
COLORS = ["#b8860b","#d4a017","#c8960c","#8b6914","#e8c547","#a07830"]
...
def clean_fig(fig, height=340):
    fig.update_layout(
        ...
        plot_bgcolor="#fffef5",
        font=dict(family="DM Sans", size=12, color="#1a1609"),
        margin=dict(l=10, r=10, t=36, b=10),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1,
                    bgcolor="rgba(0,0,0,0)", font=dict(size=11)),
        xaxis=dict(showgrid=False, showline=True, linecolor="#e8e0c8", tickfont=dict(size=11)),
        yaxis=dict(showgrid=True,  gridcolor="#f5efd0", showline=False, tickfont=dict(size=11)),
    )
    return fig
...
with st.sidebar:
    ...
    <div style="text-align:center; padding: 16px 0 20px;">
        ...
        <div style="font-weight:800; font-size:1.1rem; color:#b8860b; letter-spacing:1px; line-height:1.3;">Revenue Intelligence<br><span style="color:#1a1609;">Command Centre</span></div>
        <div style="font-size:0.68rem; color:#a09070; letter-spacing:3px; margin-top:4px;">ANALYTICS 2022–2024</div>
    </div>
    """, unsafe_allow_html=True)
...
<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:20px;">
    <div>
        <div style="font-size:1.65rem; font-weight:800; color:#1a1609; letter-spacing:-0.5px;">📊 Sales & Revenue <span style="color:#b8860b;">Intelligence</span> Dashboard</div>
        <div style="font-size:0.82rem; color:#a09070; margin-top:4px; letter-spacing:0.5px;">Real-time business performance analytics · Superstore Dataset · 2022–2024</div>
    </div>
    <div style="font-size:0.75rem; font-family:'DM Mono',monospace; color:#2563eb;
                background:#fdf8e8; padding:6px 14px; border-radius:20px; border:1px solid #e8d88a;">
<span style="color:#b8860b;">●</span> LIVE FILTERS ACTIVE
    </div>
</div>
""", unsafe_allow_html=True)
...
with r1c1:
    ...
    fig1.add_trace(go.Scatter(
        ...
        marker=dict(size=5),
        fill="tozeroy", fillcolor="rgba(184,134,11,0.07)"
    ))
    fig1.add_trace(go.Scatter(
...
with r3c1:
    ...
    fig5.add_hline(y=20, line_dash="dash", line_color="#b8860b", annotation_text="Target 20%",
...
with r3c2:
    ...
    fig6 = go.Figure(go.Pie(
        ...
        hole=0.45,
        marker=dict(colors=["#b8860b","#d4a017","#8b6914"]),
        textinfo="label+percent",
        textfont=dict(size=11),
    ))
...
.footer-avatar {{
    ...
    box-shadow: 0 0 0 4px #fdf3c0, 0 0 12px rgba(184,134,11,0.25);
    background: #fdf8e8;
}}
.footer-avatar img {{
    width: 100%;
...
.footer-name  {{ font-weight: 700; font-size: 0.95rem; color: #1a1609; line-height: 1.2; }}
.footer-role  {{ font-size: 0.72rem; color: #b8860b; letter-spacing: 2px; margin-top: 3px; font-weight: 600; }}
...
.f-btn-repo {{ background:#fdf8e8; color:#b8860b !important; border-color:#e8d88a; }}
.f-btn-repo:hover {{ background:#b8860b; color:#fff !important; border-color:#b8860b; }}
...
.footer-copy {{
    ...
    border-top: 1px solid #e8e0c8; padding-top: 12px;
}}
</style>
